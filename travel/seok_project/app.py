...
from flask import render_template   # template 폴더 안에 파일을 쓰겠다
from flask import request           # 회원정보를 제출할 때 쓰는 request, post 요청 처리
...
def register():
    ...
    if form.validate_on_submit(): # 유효성 검사, 내용 채우지 않은 항목이 있는지까지 체크
        ...
        db.session.add(usertable)   # DB 저장
...
def login():
    ...
    if form.validate_on_submit():   # 유효성 검사
        print('{}가 로그인했습니다'.format(form.data.get('userid')))
...
if __name__ == "__main__":
    ...
    app.config['SQLALCHEMY_COMMIT_ON_TEARDOWN'] = True  # 사용자에게 정보 전달완료하면 teadown. 그 때마다 커밋
